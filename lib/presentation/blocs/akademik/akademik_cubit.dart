...
import 'package:cat_akademik_kepolisian/domain/use_cases/akademik/get_akademik_questions_use_case.dart';
import 'package:cat_akademik_kepolisian/domain/use_cases/akademik/submit_akademik_use_case.dart';
...
part 'akademik_cubit.freezed.dart';

extension ScreenSizeCubitExt on BuildContext {
  AkademikCubit get readAkademikCubit => read<AkademikCubit>();
  AkademikCubit get watchAkademikCubit => watch<AkademikCubit>();
}

@injectable
class AkademikCubit extends Cubit<AkademikState> {
  final GetAkademikQuestionsUseCase getAkademikQuestionsUseCase;
  final SubmitAkademikQuestionsUseCase submitAkademikQuestionsUseCase;

  AkademikCubit(
    this.getAkademikQuestionsUseCase,
    this.submitAkademikQuestionsUseCase,
  ) : super(AkademikState.initial());

  void toQuestion(int index) {
    index == state.akademikQustions.length
  ...
  void getAkademikQuestions() async {
    emit(state.copyWith(akademikQustionsState: const ViewState.loading()));

    final result = await getAkademikQuestionsUseCase.call(EmptyParams());
    ...
    result.when(
      success: (data) {
        ...
        emit(
          state.copyWith(
            akademikQustionsState: const ViewState.success(),
            akademikQustions: shuffledData,
      ...
      error: (message, errorCode) => emit(state.copyWith(
          akademikQustionsState: ViewState.error(message, errorCode))),
    );
  }

  void mutateAnswer(QuestionEntity question, String optionId) {
    final data = state.akademikQustions.toList();
    final questionIndex =
    ...
    emit(state.copyWith(akademikQustions: data));
  }
  ...
  void submit() async {
    ...
    final result =
        await submitAkademikQuestionsUseCase.call(state.akademikQustions);
